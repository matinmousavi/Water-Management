		...
		const initializeDemoSession = async () => {
			try {
				...
				if (response.success && Array.isArray(response.users)) {
					const uniqueUsersByRole = Array.from(new Map(response.users.map(user => [user.role, user])).values())

					setDemoUsers(uniqueUsersByRole)
								...
								{demoUsers.length > 0 && (
									...
									>
										...
										<Flex gap={8} wrap='wrap'>
											{demoUsers.map(user => (
												<Button key={user.role} size='small' onClick={() => handleDemoUserSelect(user.mobile)}>
