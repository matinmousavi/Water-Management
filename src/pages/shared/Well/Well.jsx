...
const Well = () => {
    ...
    const api = useAPI()
    const { user, isAdmin, isIrrigator, isLandOwner } = useUser()
    ...
    const filterWells = wellsApi.data?.wells?.filter(
        well => well?.irrigator?._id === user._id
    )
        ...
        <WellProvider value={contextValue}>
            ...
            <Flex vertical gap={20}>
                {isMobile && isIrrigator && (
                    <WellMobile
                        wellId={well?._id}
                        setIrrigatorWells={setIrrigatorWells}
                        irrigatorWells={irrigatorWells}
                        filterWells={filterWells}
                    />
                )}
