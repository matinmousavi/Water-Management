...
const TimePickerSheet = ({
	...
	const backwardMinutes = useMemo(
		() => toPositiveInteger(marginMinutesBackward) + toPositiveInteger(marginHoursBackward) * 60,
		[marginHoursBackward, marginMinutesBackward]
	)

	const forwardMinutes = useMemo(
		() => toPositiveInteger(marginMinutesForward) + toPositiveInteger(marginHoursForward) * 60,
	...
	const minuteRange = useMemo(() => {
		...
		return Array.from({ length: Math.max(total, 1) }, (_, idx) => base.add(idx, 'minute').second(0).millisecond(0))
	...
	const emitSelection = useCallback(
		index => {
			...
			// 🔹 لاگ زمان انتخاب‌شده
			console.log('[TimePickerSheet] انتخاب کاربر:', normalizedCandidate.format('YYYY-MM-DD HH:mm:ss'))
	...
	const handleConfirm = useCallback(() => {
		...
		const isoString = normalizedCandidate.toDate().toISOString()

		// 🔹 لاگ خروجی نهایی
		console.log('[TimePickerSheet] زمان تأیید‌شده (ISO):', isoString)
	...
	return (
		<BottomSheetModal open={open} height={height} onClose={onClose} title={title} okText={confirmText} closeText={cancelText} onSubmit={handleConfirm}>
