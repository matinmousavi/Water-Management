 ...
 * @param {{ landId?: string | null, landGroupId?: string | null, workspaceId?: string | null }} params
...
export const calculateTotalDuration = async ({
    ...
    if (landGroupId) {
        irrigations = await Irrigation.find({
            ...workspaceFilter,
        ...
        irrigations.forEach(irrigation => {
            ...
            if (!uniqueLogsMap.has(key)) {
                uniqueLogsMap.set(key, irrigation)
            }
        })
 ...
 */
export const checkIrrigationConflict = async ({
    wellId,
    landIds = [],
    excludeId = null,
    startedAt,
    ...
    if (isOngoing) {
        if (irrigations.some(irrigation => irrigation.isOngoing)) {
            return 'این چاه در حال حاضر در حال آبیاری است.'
        }
    ...
    if (well?.landGroups?.length) {
        for (const group of well.landGroups) {
            ...
            group.lands.forEach(land => {
                groupLandIdsSet.add(land.toString())
            })
        }
    }

    const allLandIds = Array.from(groupLandIdsSet).concat(
        landIds.map(landId => landId.toString())
    )
    ...
    for (const land of lands) {
        landMap.set(land._id.toString(), land)
    }
    ...
    for (const irrigation of irrigations) {
            ...
            for (const landId of group.lands) {
                ...
                const hasValidMembership = land.groupMemberships.some(
                    membership =>
                        ...
                        (!membership.endDate ||
                            membership.endDate >= irrigation.startedAt)
                )

                if (hasValidMembership) {
                    irrigationLandIds.push(landId.toString())
                }
            }
        ...
        const overlapLand = landIds.some(landId =>
            irrigationLandIds.includes(landId.toString())
        )

        if (!overlapLand) continue

        const existingStart = new Date(irrigation.startedAt)
        const existingEnd = irrigation.endedAt
            ? new Date(irrigation.endedAt)
            : null
        ...
        if (
            ...
            formatTimeHHmm(existingStart) === formatTimeHHmm(newStart)
        ) {
...
/**
 * @param {Array<Object>} irrigations
 * @returns {Array<Object>} uniqueLogs
 */
export const groupIrrigationLogs = (irrigations = []) => {
    const map = new Map()

    for (const ir of irrigations) {
        if (!ir?.startedAt) continue

        const startedTs = new Date(ir.startedAt).getTime()

        if (Number.isNaN(startedTs)) continue

        const isOngoing = Boolean(ir.isOngoing)
        let key = null

        if (isOngoing) {
            const createdBy = ir.createdBy ? String(ir.createdBy) : 'unknown'
            key = `ongoing|${createdBy}|${startedTs}`
        } else {
            if (!ir.endedAt) continue

            const endedTs = new Date(ir.endedAt).getTime()

            if (Number.isNaN(endedTs)) continue

            const createdBy = ir.createdBy ? String(ir.createdBy) : 'unknown'
            key = `done|${createdBy}|${startedTs}|${endedTs}`
        }
