...
const sendIrrigationNotificationToLandOwner = async ({ landId, irrigationDocument, endedAt, currentUser, workspaceId }) => {
	const landDocument = await Land.findOne({
		_id: landId,
		workspaceId,
...
const updateGroupIrrigationLogs = async ({ groupIrrigationDocuments, requestBody, currentUser, workspaceId }) => {
	...
	for (const irrigationDocument of groupIrrigationDocuments) {
		...
		Object.entries(requestBody).forEach(([fieldName, fieldValue]) => {
			if (!['startDate', 'startTime', 'endDate', 'endTime', 'createdBy', 'workspaceId'].includes(fieldName)) {
	...
	for (const irrigation of updatedIrrigations) {
		irrigation.landGroupTitle = await getLandGroupTitle(irrigation, irrigation.well, workspaceId)
...
router.get('/', async (req, res) => {
	try {
		...
		for (const irrigation of irrigations) {
			irrigation.landGroupTitle = await getLandGroupTitle(irrigation, irrigation.well, req.workspaceId)
	...
	try {
		...
		if (irrigationDocument.landGroup) {
			...
			const responseObject = {
				...
				landGroupTitle: await getLandGroupTitle(representative, representative.well, req.workspaceId),
				well: {
		...
		updatedIrrigation.landGroupTitle = await getLandGroupTitle(updatedIrrigation, updatedIrrigation.well, req.workspaceId)
