...
router.get('/day/:date', async (req, res) => {
    try {
        ...
        const daysPassedToday =
            diffInDays(
                today,
                startDate
            )

        const todayDayInCycle =
            (daysPassedToday %
                well.cycleDays) +
            1
        ...
        for (const key in grouped) {
            ...
            const targetFilter =
                ...
                'land'
                    ...
                    : {
                          landGroup:
                              schedGroup.landGroup,
                          isGroupLog:
                              true,
                      }

            const allSchedulesInCycle =
                await Schedule.find({
                    workspaceId:
                        req.workspaceId,
                    well: wellId,
            ...
            for (const schedule of grouped[
                key
            ]) {
                let irrigationInProgress =
                    false

                let irrigationStartedAt =
                    null

                let irrigationEndsAt =
                    null
...
router.post('/', async (req, res) => {
    try {
        ...
        if (
            ...
            return res.status(400).json({
                message:
                    'زمان‌بندی با ساعت خاموشی چاه تداخل دارد.',
            })
        }
        ...
        for (const s of existingSchedules) {
            ...
            if (
                ...
                return res.status(400).json({
                    message: `زمان‌بندی با زمان‌بندی دیگر "${s.title}" تداخل دارد.`,
                })
            }
        }
        ...
        if (
            ...
            return res.status(400).json({
                message:
                    'اطلاعات زمین یا گروه نامعتبر است.',
            })
        }
