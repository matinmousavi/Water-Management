...
router.get('/', async (req, res) => {
    try {
        ...
        for (const group of well.landGroups) {
            ...
            const totalSchedulesInCycle =
                schedules.length

            const totalRequiredMs =
                sumScheduleDurationsMs(schedules)

            const irrigationProjection = getProjection(req)

            const irrigationsRaw =
                await Irrigation.find(
                    {
                        workspaceId: req.workspaceId,
                        well: wellId,
                        landGroup: group.groupId,
            ...
            const irrigations =
                groupIrrigationLogs(irrigationsRaw)
            ...
            group.requiredWater =
                waterMetrics.requiredWater

            group.receivedWater =
                waterMetrics.receivedWater

            group.remainingWater =
                waterMetrics.remainingWater

            group.totalSchedulesInCycle =
                totalSchedulesInCycle

            group.receivedWaterInCycle =
                waterMetrics.receivedWater
        }

        return res.status(200).json({
            landGroups: well.landGroups || [],
        })
    } catch (err) {
        console.error(err)
...
router.post('/', async (req, res) => {
    try {
        ...
        if (invalidLands.length > 0) {
            return res.status(400).json({
                message:
                    'برخی از زمین‌ها به این چاه تعلق ندارند.',
            })
        }
...
router.get('/:groupId', async (req, res) => {
    try {
        ...
        const mergedIrrigations =
            groupIrrigationLogs(irrigationsRaw)

        const irrigationsDone =
            mergedIrrigations.filter(
                ir => !ir.isOngoing && ir.endedAt
            )
        ...
        if (ongoing) {
            ...
            if (isGroup) {
                ...
                const targetGroup =
                    well.landGroups.find(g =>
                        g.groupId.equals(targetGroupId)
                    )

                const title =
                    targetGroup?.title || ''

                let targetRequiredWater =
                    requiredWater

                let targetReceivedWater =
                    receivedWater

                let targetRemainingWater =
                    remainingWater

                if (
                    !targetGroupId.equals(groupId)
                ) {
                    ...
                    const targetMergedIrrigations =
                        groupIrrigationLogs(
                            targetIrrigationsRaw
                        )

                    const targetIrrigationsDone =
                        targetMergedIrrigations.filter(
                            ir =>
                                !ir.isOngoing &&
                                ir.endedAt
                        )

                    const targetReceivedMs =
                        sumIrrigationDurationsMs(
                            targetIrrigationsDone
                        )

                    const targetRequiredMs =
                        sumScheduleDurationsMs(
                            targetSchedules
                        )
                    ...
                    targetRequiredWater =
                        targetMetrics.requiredWater

                    targetReceivedWater =
                        targetMetrics.receivedWater

                    targetRemainingWater =
                        targetMetrics.remainingWater
                }
...
router.delete('/:groupId', async (req, res) => {
    try {
        ...
        const beforeCount =
            well.landGroups.length

        well.landGroups =
            well.landGroups.filter(
                g => !g.groupId.equals(groupId)
            )
