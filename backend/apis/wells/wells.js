...
const getLandGroupTitle = (landGroupId, well) => {
    ...
    const group = well.landGroups.find(
        g => g.groupId.toString() === landGroupId.toString()
    )
    ...
    try {
        ...
        if (filters) {
            try {
                ...
                return res
                    .status(400)
                    .json({ message: 'پارامتر filters نامعتبر است.' })
            }
        }
        ...
        wells = await Promise.all(
            wells.map(async well => {
                const includeLogs =
                    !fields || requestedFields.includes('logs')

                const includeNotes =
                    !fields || requestedFields.includes('notes')

                const includeLands =
                    !fields || requestedFields.includes('lands')
                ...
                if (includeLogs) {
                    ...
                    const logsWithGroups = logs.map(log => ({
                        ...log,
                        landGroupTitle: getLandGroupTitle(
                            log.landGroup,
                            well
                        ),
                    }))
...
router.get('/:wellId', async (req, res) => {
    try {
        ...
        if (includeLogs) {
            let logs = await Irrigation.find({
                workspaceId: req.workspaceId,
                well: wellId,
            })
                .populate({
            ...
            logs = await Promise.all(
                logs.map(async log => {
                    const landGroupTitle = getLandGroupTitle(
                        log.landGroup,
                        well
                    )
                    ...
                    if (!log.endedAt) {
                        ...
                        const previousIrrigations =
                            await Irrigation.find({
                                workspaceId: req.workspaceId,
                                well: wellId,
                                landGroup:
                                    log.landGroup || log.land,
                                isGroupLog:
                                    log.isGroupLog || false,
                                endedAt: { $exists: true },
                            }).lean()
...
router.post('/', async (req, res) => {
    try {
        ...
        newWell = await newWell.populate(
            'irrigator',
            'fullName'
        )
        ...
        const representation = {
            ...
            irrigator: newWell.irrigator
                ...
                : null,
            landsCount: Array.isArray(newWell.lands)
                ? newWell.lands.length
                : 0,
        }
        ...
        if (err.code === 11000) {
            ...
            const fieldName =
                fieldTranslations.wells[field] || field
        ...
        if (err.name === 'ValidationError') {
            ...
            const field = firstError.path.includes('.')
                ? firstError.path.split('.')[1]
                : firstError.path
...
router.patch('/:wellId', async (req, res) => {
    try {
        ...
        if (updates.lands && Array.isArray(updates.lands)) {
            const removedLands = well.lands.filter(
                existingLand =>
                    !updates.lands.includes(
                        existingLand.toString()
                    )
            )

            if (
                removedLands.length > 0 &&
                well.landGroups &&
                well.landGroups.length > 0
            ) {
                removedLands.forEach(removedLandId => {
                    const groupIndex =
                        well.landGroups.findIndex(g =>
                            g.lands.some(
                                l =>
                                    l.toString() ===
                                    removedLandId.toString()
                            )
                        )

                    if (groupIndex !== -1) {
                        well.landGroups[groupIndex].lands =
                            well.landGroups[
                                groupIndex
                            ].lands.filter(
                                id =>
        ...
        const updated = await Well.findOne({
            _id: wellId,
            workspaceId: req.workspaceId,
        })
            .populate('irrigator')
