...
router.get('/', async (req, res) => {
    try {
        ...
        if (filters) {
            try {
                ...
                return res.status(400).json({
                    error: 'پارامتر filters نامعتبر است.',
                })
            }
        }
        ...
        return res.status(200).json({
            notes: notes.map(note => pickFields(note, fields)),
        })
    } catch (err) {
...
router.post('/', async (req, res) => {
    try {
        ...
        if (!['personal', 'well', 'land', 'landGroup'].includes(type)) {
            return res.status(400).json({
                error: 'نوع یادداشت معتبر نیست',
            })
        }

        if (!reference) {
            return res.status(400).json({
                error: 'شناسه مرجع الزامی است',
            })
        }
        ...
        const refData = await getReference(
            type,
            reference,
            req.workspaceId
        )
...
router.get('/:noteId', async (req, res) => {
    try {
        ...
        note.reference = await getReference(
            note.type,
            note.reference,
            req.workspaceId
        )
        ...
        return res.status(200).json({
            note: pickFields(note, fields),
        })
    } catch (err) {
        console.error(err)

        return res.status(500).json({
            error: 'خطا در دریافت یادداشت.',
        })
    }
})
...
router.delete('/:noteId', async (req, res) => {
    try {
        ...
        if (
            ...
            return res.status(403).json({
                error: 'شما اجازه حذف این یادداشت را ندارید',
            })
        }
