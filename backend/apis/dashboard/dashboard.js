...
const getReference = async (type, refId, workspaceId) => {
    if (!type || !refId || !workspaceId) return null
    ...
    if (type === 'landGroup') {
        ...
        for (const well of wells) {
            const group = well.landGroups.find(
                g => g.groupId.toString() === refId.toString()
            )
...
function diffInMinutes(later, earlier) {
    return Math.trunc(
        (later.getTime() - earlier.getTime()) / MINUTE_IN_MS
    )
}
...
function isLogOutOfSchedule(log, schedule, bufferMinutes = 2) {
    const { schedStart, schedEnd } = scheduleTimesForDate(
        schedule,
    ...
    return (
        logStart.getTime() < beforeAllowed.getTime() ||
        logEnd.getTime() > afterAllowed.getTime()
    )
}
...
router.get('/', async (req, res) => {
    try {
        ...
        const sortedLogs = irrigations.sort(
        ...
        for (const log of sortedLogs) {
            ...
            const well = wells.find(
                w => w._id.toString() === log.well._id.toString()
            )
            ...
            const daysPassed = Math.floor(
                (logStartDate.getTime() - startDate.getTime()) /
                    (1000 * 60 * 60 * 24)
            )

            const dayInCycle =
                (daysPassed % well.cycleDays) + 1
            ...
            if (!scheduleCache[cacheKeyForStatus]) {
                scheduleCache[cacheKeyForStatus] =
                    await Schedule.find(
                        {
                            well: well._id,
                            workspaceId,
            ...
            if (!scheduleCache[cacheKeyForProgress]) {
                scheduleCache[cacheKeyForProgress] =
                    await Schedule.find(
                        {
                            well: well._id,
                            workspaceId,
            ...
            const schedulesForStatus =
                scheduleCache[cacheKeyForStatus].filter(
                    sch =>
                        (sch.targetType === 'land' &&
                            sch.land?.toString() ===
                                log.land?._id?.toString()) ||
                        (sch.targetType === 'group' &&
                            sch.landGroup?.toString() ===
                                log.landGroup?.toString())
                )

            const schedulesForProgress =
                scheduleCache[cacheKeyForProgress].filter(
                    sch =>
                        (sch.targetType === 'land' &&
                            sch.land?.toString() ===
                                log.land?._id?.toString()) ||
            ...
            const waterPercent =
                totalSchedMinutes > 0
                    ? Math.min(
                          100,
                          Math.round(
                              (logDuration / totalSchedMinutes) * 100
                          )
                      )
                    : 0
            ...
            if (log.isOngoing) {
                ...
                schedulesForStatus.some(sch =>
                    isLogOutOfSchedule(log, sch, 2)
                )
            ) {
            ...
            wellsData.push({
                ...
                ...(land ? { land } : {}),
                ...(landGroup ? { landGroup } : {}),
                startTime: formatTimeHHmm(
                    new Date(log.startedAt)
                ),
                endTime: log.endedAt
                    ? formatTimeHHmm(new Date(log.endedAt))
                    : null,
                status,
        ...
        const unreadNotesCount = await Note.countDocuments({
            user: req.user._id,
            workspaceId,
            isRead: false,
        })
...
router.patch('/notes/:noteId/mark-read', async (req, res) => {
    try {
        ...
        return res.status(200).json({
            message: 'یادداشت به عنوان خوانده شده علامت‌گذاری شد',
        })
    } catch (err) {
        console.error(err)
